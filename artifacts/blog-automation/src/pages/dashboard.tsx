...
import { FileText, CheckCircle, XCircle, Clock, TrendingUp, Plus, ArrowRight } from "lucide-react";
...
export default function Dashboard() {
  ...
  const { data: active, isLoading: activeLoading, isError: activeError, refetch: refetchActive } = useGetActiveArticles({
    query: { refetchInterval: 3000, queryKey: getGetActiveArticlesQueryKey() },
  });
  const statsPayload = isObjectLike(stats) ? stats : null;
  const totalArticles = asFiniteNumber(statsPayload?.totalArticles) ?? 0;
  const completedArticles = asFiniteNumber(statsPayload?.completedArticles) ?? 0;
  const articlesThisWeek = asFiniteNumber(statsPayload?.articlesThisWeek) ?? 0;
  const failedArticles = asFiniteNumber(statsPayload?.failedArticles) ?? 0;
  const successRate = asFiniteNumber(statsPayload?.successRate);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={statsLoading ? "..." : String(totalArticles)}
          icon={<FileText className="h-5 w-5 text-primary" />}
          sub="all time"
        />
        <StatCard
          label="Completed"
          value={statsLoading ? "..." : String(completedArticles)}
          icon={<CheckCircle className="h-5 w-5 text-green-500" />}
          sub={successRate != null ? `${successRate}% success rate` : "—"}
        />
        <StatCard
          label="This Week"
          value={statsLoading ? "..." : String(articlesThisWeek)}
