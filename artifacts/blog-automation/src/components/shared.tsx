...
export function CopyleaksScore({ score }: { score?: number | null }) {
  if (score == null) return <span className="text-muted-foreground text-sm">—</span>;

  const color =
    score < 10 ? "text-green-600" :
...
/**
 * Display the number of AI-signature retry loops the pipeline ran on an article.
 * 0 = article passed all AI-detection heuristics on first writing pass
 * 1 = one retry was needed (light intervention)
 * 2 = maximum retries hit (pipeline worked hard; article might still be imperfect)
 */
export function AiSignatureRetryCount({ count }: { count?: number | null }) {
  if (count == null) return <span className="text-muted-foreground text-sm">—</span>;

  const color =
    count === 0 ? "text-green-600" :
    count === 1 ? "text-amber-600" :
    "text-red-600";
